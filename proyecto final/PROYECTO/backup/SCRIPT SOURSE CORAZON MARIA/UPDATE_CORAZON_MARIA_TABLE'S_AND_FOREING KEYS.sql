...
CREATE DATABASE CORAZON_MARIA_UPDATE_FINAL
USE CORAZON_MARIA_UPDATE_FINAL
...
CREATE TABLE Contrato(
    ...
    estado CHAR(1) NOT NULL
	--motivo_Anulacion text,--para auditoria 
	--fechaHora_Anulacion datetime,--para auditoria
	--usuario_Anulacion varchar(100)--para auditoria 
);
...
--sp_rename 'Comprobante.hota', 'hora', 'COLUMN';
...
--esto no se necesita las anulaciones se realizaran en el mismo contrato con el estado 
--el estado sera P:proceso F:finalizado A:anulado 
--tendremos las columnas de motivo de anulacion, la fecha y hora , y el usuario de systema que lo realizo 
--finalmente ahi se insertaran si se modifica el estado a A: anulado 
...
--A LAS TABLAS DE AUDITORIA NO SE LES CREA FORANEAS 
...
ALTER TABLE InventarioMateriaPrima
	...
	--no va persona 
--,CONSTRAINT FK_InventarioMateriaPrima_Persona
 --   FOREIGN KEY (idPersona)
 --   REFERENCES Persona(idPersona);
